...
LOG_MODULE_REGISTER(app_usb, CONFIG_APP_USB_LOG_LEVEL);
...
USBD_DEVICE_DEFINE(cdc_acm_serial,
		   ...
		   CONFIG_APP_USB_CDC_ACM_SERIAL_VID, CONFIG_APP_USB_CDC_ACM_SERIAL_PID);

USBD_DESC_LANG_DEFINE(cdc_acm_serial_lang);
USBD_DESC_MANUFACTURER_DEFINE(cdc_acm_serial_mfr, CONFIG_APP_USB_CDC_ACM_SERIAL_MANUFACTURER_STRING);
USBD_DESC_PRODUCT_DEFINE(cdc_acm_serial_product, CONFIG_APP_USB_CDC_ACM_SERIAL_PRODUCT_STRING);
IF_ENABLED(CONFIG_HWINFO, (USBD_DESC_SERIAL_NUMBER_DEFINE(cdc_acm_serial_sn)));

USBD_DESC_CONFIG_DEFINE(fs_cfg_desc, "FS Configuration");
USBD_DESC_CONFIG_DEFINE(hs_cfg_desc, "HS Configuration");

static const uint8_t attributes = IS_ENABLED(CONFIG_APP_USB_CDC_ACM_SERIAL_SELF_POWERED) ?
				  USB_SCD_SELF_POWERED : 0;

USBD_CONFIGURATION_DEFINE(cdc_acm_serial_fs_config,
			  attributes,
			  CONFIG_APP_USB_CDC_ACM_SERIAL_MAX_POWER, &fs_cfg_desc);

USBD_CONFIGURATION_DEFINE(cdc_acm_serial_hs_config,
			  attributes,
			  CONFIG_APP_USB_CDC_ACM_SERIAL_MAX_POWER, &hs_cfg_desc);

static struct usbd_context *usbd_ctx = &cdc_acm_serial;
			      ...
			      const enum usbd_speed speed)
{
	...
	err = usbd_register_class(usbd_ctx, "cdc_acm_0", speed, 1);
	if (err) {
...
static int cdc_acm_serial_init_device(void)
{
	...
	err = usbd_add_descriptor(usbd_ctx, &cdc_acm_serial_lang);
	if (err) {
		LOG_ERR("Failed to initialize %s (%d)", "language descriptor", err);
		return err;
	}

	err = usbd_add_descriptor(usbd_ctx, &cdc_acm_serial_mfr);
	if (err) {
		LOG_ERR("Failed to initialize %s (%d)", "manufacturer descriptor", err);
		return err;
	}

	err = usbd_add_descriptor(usbd_ctx, &cdc_acm_serial_product);
	if (err) {
	...
	IF_ENABLED(CONFIG_HWINFO, (
		err = usbd_add_descriptor(usbd_ctx, &cdc_acm_serial_sn);
	))
	if (err) {
	...
	if (USBD_SUPPORTS_HIGH_SPEED &&
		...
		err = register_cdc_acm_0(usbd_ctx, USBD_SPEED_HS);
		if (err) {
			return err;
		}
	}

	err = register_cdc_acm_0(usbd_ctx, USBD_SPEED_FS);
	if (err) {
		return err;
	}
